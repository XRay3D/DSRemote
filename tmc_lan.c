...
struct tmcdev* tmclan_open(const char* host_or_ip) {
    ...
    if(setsockopt(sockfd, IPPROTO_TCP, TCP_NODELAY, (void*)&tcp_nodelay, sizeof tcp_nodelay)
        == -1)
        return NULL;
...
int tmclan_write(struct tmcdev* tmc_device __attribute__((unused)), const char* cmd) {
    ...
    if(!(!strncmp(buf, ":TRIG:STAT?", 11) ||  /* don't print these commands to the console */
           ...
           !strncmp(buf, ":WAV:DATA?", 10) || !strncmp(buf, ":WAV:MODE NORM", 14)
           || !strncmp(buf, ":WAV:FORM BYTE", 14) || !strncmp(buf, ":WAV:SOUR CHAN", 14)
           || !strncmp(buf, ":TRIG:SWE?", 10) || !strncmp(buf, ":ACQ:SRAT?", 10)
           || !strncmp(buf, ":ACQ:MDEP?", 10) || !strncmp(buf, ":MEAS:COUN:VAL?", 15)
           || !strncmp(buf, ":FUNC:WREC:OPER?", 16) || !strncmp(buf, ":FUNC:WREP:OPER?", 16)
           || !strncmp(buf, ":FUNC:WREP:FMAX?", 16) || !strncmp(buf, ":FUNC:WREC:FMAX?", 16)
           || !strncmp(buf, ":FUNC:WREP:FCUR?", 16) || !strncmp(buf, ":WAV:XOR?", 9)))
